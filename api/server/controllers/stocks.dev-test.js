...
describe('Stocks', () => {
  ...
  describe('/GET filtering', () => {
    it('it should GET stocks that start with letter f and have a TotalScore >= 7 ', (done) => {
      chai.request(server)
        .get('/api/stocks/findAndCountAll?take=10&skip=0&page=1&pageSize=10&sort[0][field]=Name&sort[0][dir]=asc&filter[logic]=and&filter[filters][0][field]=Name&filter[filters][0][operator]=startswith&filter[filters][0][value]=F&filter[filters][1][field]=TotalScore&filter[filters][1][operator]=gte&filter[filters][1][value]=7')
        .end((err, res) => {
          res.should.have.status(200);
          res.body.should.be.a('object');
          res.body.should.have.property('data');
